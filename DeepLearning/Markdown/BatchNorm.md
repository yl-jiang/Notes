...
def batch_norm(X, gamma, beta, moving_mean, moving_var, is_training=True, eps=1e-5, momentum=0.9,):
    ...
    if len(X.shape) == 2:
        ...
        # cross batch计算channel上的均值，再reshape为[1, c, 1, 1]方便broadcast以计算var
...
[原文链接](https://blog.csdn.net/debug_moner/java/article/details/104477896)